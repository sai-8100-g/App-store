...
class AppStore extends Component {
  state = {
    initialAppsList: appsList,
    initialTabId: '',
  }

  OnSearchApps = event => {
    const {initialAppsList} = this.state

    if (event.target.value.length === 0) {
      this.setState({initialAppsList: appsList})
    } else {
      this.setState({
        initialAppsList: initialAppsList.filter(eachObj =>
          eachObj.appName
            .toLocaleLowerCase()
  ...
  filteringApps = tabId => {
    const filteredApps = appsList.filter(eachObj => eachObj.category === tabId)
    this.setState({initialAppsList: filteredApps, initialTabId: tabId})
  }

  render() {
    const {initialAppsList, initialTabId, searchInput} = this.state
      ...
      <div className="main-container">
        <div className="content-container">
            ...
            <ul className="tabs-ul">
              {tabsList.map(eachObj => (
                <TabItem
                  listOfTabs={eachObj}
                  key={eachObj.tabId}
                  filteringApps={this.filteringApps}
                  isActive={initialTabId === eachObj.tabId}
                />
              ))}
            </ul>
          </div>
          <div className="app-ul">
            <ul>
              {initialAppsList.map(eachObj => (
                <AppItem appsList={eachObj} key={eachObj.appId} />
              ))}
            </ul>
          </div>
        </div>
